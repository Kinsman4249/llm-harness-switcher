...
|-- model-profiles/                 - shipped model profile example (gemma4-e2b.sh) + README
|-- templates/                      - checked-in artifacts install.sh copies out to their destinations
|   |-- bin/                        - scripts copied to $BIN_DIR
|   |   |-- start-local-model.sh    - kilo-mode launcher template: pick a model, start it, sync the Kilo provider
|   |   |-- start-local-model-lib.sh- function library sourced by start-local-model.sh (ships alongside it)
...
|   |-- desktop/                    - .desktop launcher entries copied to $DESKTOP_DIR
...
`-- docs/                           - the manual, split into chapters (installation/models/usage/context-sizing/troubleshooting)
```
...
(`start-local-model.sh` + its `start-local-model-lib.sh`, `sync-local-model.sh`,
their desktop wrappers) are copied from `templates/bin/` the same way. None of
them are checked into the repo in final form, don't hand-edit them, re-run
`install.sh` to change any of their flags. The classic-mode service units
shipped in `templates/systemd/` (`litellm-ollama-box.service` and, optionally,
`distrobox-reminder.service`) are a documented manual option only -
`install.sh` installs neither of them and **auto-starts nothing** (see below).
